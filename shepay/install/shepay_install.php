...
<div class="container"><br>
    <div class="row">
        <div class="col-xs-12 col-sm-10 col-md-8 col-lg-6 center-block text-center" style="float: none;">
            <div class="alert alert-success" role="alert"><center>易用、好用、耐用、实用，都潮汇系统更懂你...</center></div>
        </div>
...
<div class="panel panel-black">
	...
	<div class="panel-heading" style="background: #15A638;">
		<h3 class="panel-title" align="center">都潮汇系统-安装</h3>
	</div>
	<div class="panel-body">
		<p style="text-align: center"><iframe class="video_iframe" style="z-index:1;" src="http://v.qq.com/iframe/player.html?vid=l0831ltmo9a&amp;width=300&amp;height=169&amp;auto=0" allowfullscreen="" frameborder="0" height="169" width="300"></iframe></p>
		<?php if ($installed) { ?>
		<div class="alert alert-warning">您已经安装过都潮汇系统，如需重新安装请删除<font color=red> “shepay/install/shepay.lock” </font>文件后再安装！</div>
		<?php }else{?>
...
<p><span><a class="btn btn-primary" href="shepay_install.php?do=0">上一步</a></span>
<span style="float:right"><a class="btn btn-primary" href="shepay_install.php?do=2" align="right">下一步</a></span></p>
</div>
...
echo <<<HTML
  ...
  <center>如果已事先填写好config.php文件中相关数据库配置，请 <a href="?do=3&jump=1">点击此处</a> 跳过这一步。<br><font color="red">安装后倘若运行出错，请切换PHP版本至正常运行为止！</font><br>都潮汇系统官方交流群：867211168。</center>
HTML;
?>
	</div>
</div>
	...
	if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
		...
		if(!$con=DB::connect($dbconfig['host'],$dbconfig['user'],$dbconfig['pwd'],$dbconfig['dbname'],$dbconfig['port'])){
			...
			echo '<div class="alert alert-success">数据库配置文件保存成功！</div>';
                if (DB::query("select * from card_config where 1") == FALSE) echo '<p align="right"><a class="btn btn-primary btn-block" href="?do=4">创建数据表>></a></p>';
                else echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过都潮汇系统</div>
...
?>";
		if(!$con=DB::connect($db_host,$db_user,$db_pwd,$db_name,$db_port)){
			...
			if(DB::query("select * from ayangw_config where 1")==FALSE)
				...
				echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过都潮汇系统</div>
				<div class="list-group-item">
					<a href="?do=6" class="btn btn-block btn-info">跳过安装</a>
				</div>
				<div class="list-group-item">
...
<div class="progress progress-striped">
  <div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: 70%">
	...
	<div class="panel-body">
<?php
if(defined("SAE_ACCESSKEY"))include_once '../shepay/sae.php';
else include_once '../config.php';
if(!$dbconfig['user']||!$dbconfig['pwd']||!$dbconfig['dbname']) {
	echo '<div class="alert alert-danger">请先填写好数据库并保存后再安装！<hr/><a href="javascript:history.back(-1)"><< 返回上一页</a></div>';
} else {
	require './db.class.php';
	$sql=file_get_contents("shepay.sql");
	$sql=explode(';',$sql);
...
if($e==0) {
	echo '<div class="alert alert-success">安装成功！<br/>SQL成功'.$t.'句/失败'.$e.'句</div><p align="right"><a class="btn btn-block btn-primary" href="shepay_install.php?do=5">下一步>></a></p>';
} else {
	echo '<div class="alert alert-danger">安装失败<br/>SQL成功'.$t.'句/失败'.$e.'句<br/>错误信息：'.$error.'</div><p align="right"><a class="btn btn-block btn-primary" href="shepay_install.php?do=4">点此进行重试</a></p>';
}
?>
	</div>
</div>
...
<div class="progress progress-striped">
  <div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: 100%">
	...
	<div class="panel-body">
<?php
	@file_put_contents("shepay.lock",'欢迎使用都潮汇系统，这是安装锁文件！没有安全何来支付？2019，我们依旧在努力！官方QQ交流群：867211168');
	echo '<div class="alert alert-info"><font color="green">恭喜，都潮汇系统已安装完成！（安装后倘若运行出错，请切换PHP版本至正常运行为止！）初始管理账号和密码是:admin/admin</font><br/><br/><a href="/">>>网站首页</a>｜<a href="/admin/">>>后台管理</a><hr/>更多设置选项请登录后台管理进行修改，未授权用户可安装体验，但仅可预览部分内容，无法修改站点设置，如需正式使用，请在确认服务器可以运行本程序后后，尽快联系深海QQ：779887502，购买正版授权，感谢您对深海团队的支持！<br/><br/><font color="#FF0033">如果你的空间不支持本地文件读写，请自行在shepay/install/ 目录下建立 shepay.lock 空文件！</font></div>';
?>
</div>
</div>
