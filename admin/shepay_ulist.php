...
<div class="card-header"><h3 class="panel-title">添加商户</h3></div>';
echo '<div class="card-body">';
echo '<form action="./shepay_ulist.php?my=add_submit" method="POST">
...
value="确定添加"></form>';
echo '<br/><a href="./shepay_ulist.php">>>返回商户列表</a>';
echo '</div></div>';
}
elseif($my=='edit')
{
...
echo '<div class="card card-primary">	
			<div class="card-header"><h3 class="panel-title">修改商户信息</h3></div>';
echo '<div class="card-body">';
echo '<form action="./shepay_ulist.php?my=edit_submit&id='.$id.'" method="POST">
<div class="form-group">
...
$pid=$DB->lastInsertId();
if($sds){
	showmsg('添加商户成功！商户ID：'.$pid.'<br/>密钥：'.$key.'<br/><br/><a href="./shepay_ulist.php">>>返回商户列表</a>',1);
}else
...
if($DB->exec($sql)||$sqs)
	showmsg('修改商户信息成功！<br/><br/><a href="./shepay_ulist.php">>>返回商户列表</a>',1);
else
...
if($DB->exec($sql))
	showmsg('删除商户成功！<br/><br/><a href="./shepay_ulist.php">>>返回商户列表</a>',1);
else
...
echo '<form action="shepay_ulist.php" method="GET" class="form-inline"><input type="hidden" name="my" value="search">
...
<div class="card-header">
  ...
  <button type="submit" class="btn btn-primary">搜索</button>&nbsp;<a href="./shepay_ulist.php?my=add" class="btn btn-success">添加商户</a>&nbsp;<a href="./shepay_plist.php" class="btn btn-success">合作者商户管理</a>
</form>';
...
?>
      <div class="table-responsive">
        <table class="table table-striped">
          <thead><tr><th>商户号</th><th>密钥</th><th>余额</th><th>结算账号/真实姓名</th><th>域名/添加时间</th><th>状态</th><th>操作</th></tr></thead>
          <tbody>
<?php
$pagesize=30;
...
if ($page>1)
{
echo '<li><a href="shepay_ulist.php?page='.$first.$link.'">首页</a></li>';
echo '<li><a href="shepay_ulist.php?page='.$prev.$link.'">&laquo;</a></li>';
} else {
echo '<li class="disabled"><a>首页</a></li>';
echo '<li class="disabled"><a>&laquo;</a></li>';
}
for ($i=1;$i<$page;$i++)
echo '<li><a href="shepay_ulist.php?page='.$i.$link.'">'.$i .'</a></li>';
...
if ($page<$pages)
{
echo '<li><a href="shepay_ulist.php?page='.$next.$link.'">&raquo;</a></li>';
echo '<li><a href="shepay_ulist.php?page='.$last.$link.'">尾页</a></li>';
} else {
