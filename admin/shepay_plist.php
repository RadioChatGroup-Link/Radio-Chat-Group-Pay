...
if($my=='add')
{
echo '<div class="card card-primary"><div class="card-header"><h3 class="panel-title">添加合作者</h3></div>';
echo '<div class="card-body">';
echo '<form action="./shepay_plist.php?my=add_submit" method="POST">
...
value="确定添加"></form>';
echo '<br/><a href="./shepay_plist.php">>>返回合作者列表</a>';
echo '</div></div>';
}
elseif($my=='edit')
{
...
echo '<div class="card card-primary"><div class="card-header"><h3 class="panel-title">修改商户信息</h3></div>';
echo '<div class="card-body">';
echo '<form action="./shepay_plist.php?my=edit_submit&id='.$id.'" method="POST">
<div class="form-group">
<label>用户名:</label><br>
...
$pid=$DB->lastInsertId();
if($sds){
	showmsg('添加合作者身份成功！合作者身份TOKEN：'.$key.'<br/><br/><a href="./shepay_plist.php">>>返回合作者列表</a>',1);
}else
...
if($DB->exec($sql)||$sqs)
	showmsg('修改合作者身份信息成功！<br/><br/><a href="./shepay_plist.php">>>返回合作者列表</a>',1);
else
...
if($DB->exec($sql))
	showmsg('删除合作者成功！<br/><br/><a href="./shepay_plist.php">>>返回合作者列表</a>',1);
else
...
echo '<form action="shepay_plist.php" method="GET" class="form-inline"><input type="hidden" name="my" value="search">
...
<div class="card-header">
  ...
  <button type="submit" class="btn btn-primary">搜索</button>&nbsp;<a href="./shepay_plist.php?my=add" class="btn btn-success">添加合作者</a>&nbsp;<a href="./shepay_ulist.php" class="btn btn-success">普通商户管理</a>
</form>';
...
while($res = $rs->fetch())
{
echo '<tr><td><b>'.$res['id'].'</b></td><td>'.$res['user'].'</td><td>'.$res['pwd'].'</td><td>'.$res['token'].'</td><td>'.$res['regtime'].'</td><td>'.$res['level'].'</td><td>'.($res['active']==1?'<font color=green>正常</font>':'<font color=red>封禁</font>').'</td><td><a href="./shepay_plist.php?my=edit&id='.$res['id'].'" class="btn btn-xs btn-info">编辑</a>&nbsp;<a href="./shepay_plist.php?my=delete&id='.$res['id'].'" class="btn btn-xs btn-danger" onclick="return confirm(\'你确实要删除此商户吗？\');">删除</a></td></tr>';
}
?>
          </tbody>
        </table>
...
if ($page>1)
{
echo '<li><a href="shepay_plist.php?page='.$first.$link.'">首页</a></li>';
echo '<li><a href="shepay_plist.php?page='.$prev.$link.'">&laquo;</a></li>';
} else {
echo '<li class="disabled"><a>首页</a></li>';
echo '<li class="disabled"><a>&laquo;</a></li>';
}
for ($i=1;$i<$page;$i++)
echo '<li><a href="shepay_plist.php?page='.$i.$link.'">'.$i .'</a></li>';
...
if ($page<$pages)
{
echo '<li><a href="shepay_plist.php?page='.$next.$link.'">&raquo;</a></li>';
echo '<li><a href="shepay_plist.php?page='.$last.$link.'">尾页</a></li>';
} else {
