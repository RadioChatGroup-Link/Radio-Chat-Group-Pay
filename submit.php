...
if(isset($_GET['pid'])){
	...
	exit("<script language='javascript'>alert('支付请求发起站点未传送有效商户信息，请稍后重试！');history.go(-1);</script>");
}
...
if(!md5Verify($prestr, $queryArr['sign'], $userrow['key']))sysmsg($conf['key_no']);
if($userrow['active']==0)sysmsg($conf['user_no']);
...
sysmsg($conf['alipay_offtxt']);
}
if($conf['alipay_mode']==1){
	//echo "<script>window.location.href='./pay/alipay.php?trade_no={$trade_no}&sitename={$sitename}';</script>";
	//exit;
	...
	$parameter = array(
		...
		"payment_type"	=> "1", //支付方式
		"notify_url"	=> 'http://'.$conf['local_domain'].'/pay/alipay_notify.php', //服务器异步通知页面路径
		"return_url"	=> 'http://'.$_SERVER['HTTP_HOST'].'/pay/alipay_return.php', //页面跳转同步通知页面路径
...
$parameter = array(
	...
	"type" => $type,
	"notify_url"	=> 'http://'.$conf['local_domain'].'/pay/shepay_alipay_notify.php',
	"return_url"	=> 'http://'.$_SERVER['HTTP_HOST'].'/pay/shepay_alipay_return.php',
...
if($conf['alipay_mode']==3){
	echo "<script>window.location.href='./pay/codepay.php?trade_no={$trade_no}&type=alipay&sitename={$sitename}';</script>";
}
}
if($type=='wxpay'){
if($conf['wxpay_mode']==0){
sysmsg($conf['wxpay_offtxt']);
}
if($conf['wxpay_mode']==1){
	if(strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger')!==false){
		echo "<script>window.location.href='./pay/wxjspay.php?trade_no={$trade_no}';</script>";
	}elseif(checkmobile()==true){
        if($conf['wxpay_h5']==1){
        echo "<script>window.location.href='./pay/wxwappay2.php?trade_no={$trade_no}&sitename={$sitename}';</script>";
	}else{
	}
		echo "<script>window.location.href='./pay/wxwappay.php?trade_no={$trade_no}&sitename={$sitename}';</script>";
	}else{
		echo "<script>window.location.href='./pay/wxpay.php?trade_no={$trade_no}&sitename={$sitename}';</script>";
	}
...
$parameter = array(
	"pid" => trim($alipay_config['partner']),
	"type" => $type,
	"notify_url"	=> 'http://'.$conf['local_domain'].'/pay/shepay_wxpay_notify.php',
	"return_url"	=> 'http://'.$_SERVER['HTTP_HOST'].'/pay/shepay_wxpay_return.php',
	"out_trade_no"	=> $trade_no,
	"name"	=> $name,
	"money"	=> $money
);
...
if($conf['wxpay_mode']==3){
	echo "<script>window.location.href='./pay/codepay.php?trade_no={$trade_no}&type=wxpay&sitename={$sitename}';</script>";
}
}
if($type=='qqpay'){
if($conf['qqpay_mode']==0){
sysmsg($conf['qqpay_offtxt']);
}
if($conf['qqpay_mode']==1){
	echo "<script>window.location.href='./pay/qqpay.php?trade_no={$trade_no}&sitename={$sitename}';</script>";
}
...
$parameter = array(
	"pid" => trim($alipay_config['partner']),
	"type" => $type,
	"notify_url"	=> 'http://'.$conf['local_domain'].'/pay/shepay_qqpay_notify.php',
	"return_url"	=> 'http://'.$_SERVER['HTTP_HOST'].'/pay/shepay_qqpay_return.php',
	"out_trade_no"	=> $trade_no,
	"name"	=> $name,
	"money"	=> $money
);
...
if($conf['qqpay_mode']==3){
	echo "<script>window.location.href='./pay/codepay.php?trade_no={$trade_no}&type=qqpay&sitename={$sitename}';</script>";
}
}
if($type=='tenpay'){
if($conf['tenpay_mode']==0){
sysmsg($conf['tenpay_offtxt']);
}
if($conf['tenpay_mode']==1){
	...
	$reqHandler->setParameter("return_url", 'http://'.$_SERVER['HTTP_HOST'].'/pay/tenpay_return.php');
	$reqHandler->setParameter("notify_url", 'http://'.$conf['local_domain'].'/pay/tenpay_notify.php');
...
$parameter = array(
	"pid" => trim($alipay_config['partner']),
	"type" => $type,
	"notify_url"	=> 'http://'.$conf['local_domain'].'/pay/shepay_tenpay_notify.php',
	"return_url"	=> 'http://'.$_SERVER['HTTP_HOST'].'/pay/shepay_tenpay_return.php',
	"out_trade_no"	=> $trade_no,
	"name"	=> $name,
	"money"	=> $money
);
...
if($conf['tenpay_mode']==3){
	echo "<script>window.location.href='./pay/codepay.php?trade_no={$trade_no}&type=tenpay&sitename={$sitename}';</script>";
}
}
else{
	echo "<script>window.location.href='./pay/default.php?trade_no={$trade_no}&sitename={$sitename}';</script>";
}
?>
<p>正在为您跳转到支付页面，请稍候...</p>
</body>
