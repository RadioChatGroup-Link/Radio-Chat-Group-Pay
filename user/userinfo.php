...
<div class="main-panel" id="situation" value="">
 <div id="content" class="app-content" role="main">
    <div class="app-content-body ">
     ...
     <h6 class="modal-title"><i class="la la-frown-o"></i> 验证密保</h6>
     <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
    </div>
					<div class="modal-body">
<?php if($conf['verifytype']==1){?>
<div class="list-group-item">当前密保手机：<?php echo $userrow['phone']?></div>
<div class="list-group-item">
<div class="input-group">
<input type="text" name="code" placeholder="输入短信验证码" class="form-control" required>
<a class="btn btn-default" id="sendcode">获取验证码</a>
</div>
</div>
<?php }else{?>
<div class="list-group-item">当前密保邮箱：<?php echo $userrow['email']?></div>
<div class="list-group-item">
<div class="input-group">
...
<div id="embed-captcha"></div>
		...
		<div class="modal inmodal fade" id="myModal2" tabindex="-1" role="dialog" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header bg-primary">
     <h6 class="modal-title"><i class="la la-frown-o"></i> 修改密保信息</h6>
     <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
    </div>
					<div class="modal-body">
<?php if($conf['verifytype']==1){?>
...
<a class="btn btn-default" id="sendcode2">获取验证码</a>
</div>
</div>
<?php }else{?>
<div class="list-group-item">
     			...
     			<div class="col-md-12">
     				<div class="card">
     					<div class="card-header">
     						<div class="card-title">商户信息查看</div>
     					</div>
     					<div class="card-body">
     						...
     						<div class="form-groupas has-error has-feedback">
     							<label>商户密钥</label>
     								<input type="text" class="form-control input-full" value="<?php echo $userrow['key']?>" disabled><small class="form-text text-muted">请妥善保管好您的密钥,可别被坏人发现了</small>
     						</div>
	     				...
	     				<div class="card">
	     					<div class="card-header">
	     						<div class="card-title">商户信息设置</div>
	     					</div>
	     					<div class="card-body">
	     						<div class="form-groupas">
	     							<label>商户结算方式</label>
	     						...
	     						<div class="form-groupas has-error has-feedback">
	     							<label>商户结算账号</label>
	     								<input type="text" class="form-control input-full" name="account" value="<?php echo $userrow['account']?>"><small class="form-text text-muted">请注意不要填写错误哦,填错我们不负责</small>
	     						</div>
	     						<div class="form-groupas has-success">
	     							<label>商户结算姓名</label>
	     							<input type="text" class="form-control" name="username" value="<?php echo $userrow['username']?>"><small class="form-text text-muted">请务必填写真实姓名,否则出款失败后果自负</small>
	     						</div>
	     						<div class="card-action">
	     								<input type="button" id="editSettle" value="确认修改" class="btn btn-primary form-control">
	     						</div><hr>
	     						<?php if($conf['verifytype']==1){?>
										<div class="form-groupas">
											<label for="disableinput">手机号码</label>
											<input type="text" class="form-control" name="phone" id="disableinput" value="<?php echo $userrow['phone']?>" disabled="">
										</div>
										<div class="form-groupas">
											<label for="disableinput">邮箱地址</label>
											<input type="text" class="form-control" name="email" id="disableinput" value="<?php echo $userrow['email']?>" disabled="">
										</div>
										<?php }else{?>
										<div class="form-groupas mb-3">
											<label>邮箱地址</label>
											<div class="input-group">
												<input type="text" class="form-control" name="email" id="disableinput" value="<?php echo $userrow['email']?>" disabled=""><div class="input-group-append"><button class="btn btn-dark" type="button" id="checkbind">修改绑定信息</button></div>
											</div>
										</div>
										<?php }?>
	     						<div class="form-groupas has-error has-feedback">
	     							<label>联系QQ</label>
	     								<input type="text" class="form-control input-full" name="qq" value="<?php echo $userrow['qq']?>">
	     						</div>
	     						<div class="form-groupas has-success">
	     							<label>对接网站域名</label>
	     							<input type="text" class="form-control" name="url" value="<?php echo $userrow['url']?>">
	     						</div>
	     						<div class="card-action">
	     								<input type="button" id="editInfo" value="确认修改" class="btn btn-primary form-control">
	     						</div>
	     						<div class="card-action <?php echo isset($_GET['connect'])||$conf['quicklogin']!=0?'hide':null;?>">
	     							<?php if(empty($userrow['alipay_uid'])){?>
	     								<a href="oauth.php?bind=true" class="btn btn-danger form-control" target="_blank">绑定支付宝账号</a>
	     							<?php }else{?>
	     						...
	     						<div class="card-action <?php echo isset($_GET['connect'])||$conf['quicklogin']!=0?'hide':null;?>">
	     							<?php if(empty($userrow['qq_uid'])){?>
	     								<a href="connect.php?bind=true" class="btn btn-danger form-control" target="_blank">绑定QQ</a>
	     							<?php }else{?>
	     						...
	     						<div class="card-action <?php echo isset($_GET['connect'])||$conf['quicklogin']!=1?'hide':null;?>">
	     							<?php if(empty($userrow['alipay_uid'])){?>
	     								<a href="oauth.php?bind=true" class="btn btn-danger form-control" target="_blank">绑定支付宝账号</a>
	     							<?php }else{?>
	     								已绑定支付宝UID:<?php echo $userrow['alipay_uid']?>&nbsp;<a href="oauth.php?unbind=true" class="btn btn-danger btn-xs" onclick="return confirm('解绑后将无法通过支付宝一键登录，是否确定解绑？');">解绑账号</a>
	     						...
	     						<div class="card-action <?php echo isset($_GET['connect'])||$conf['quicklogin']!=2?'hide':null;?>">
	     							<?php if(empty($userrow['qq_uid'])){?>
	     								<a href="connect.php?bind=true" class="btn btn-danger form-control" target="_blank">绑定QQ</a>
	     							<?php }else{?>
	     								已绑定QQ互联Openid:<?php echo $userrow['qq_uid']?>&nbsp;<a href="connect.php?unbind=true" class="btn btn-danger btn-xs" onclick="return confirm('解绑后将无法通过支付宝一键登录，是否确定解绑？');">解绑账号</a>
